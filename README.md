...
- **Processamento:**
    1.  **Merge Inteligente:** Inner Join entre SteamDB (`sid` ou `appid`) e SteamSpy (`appid`).
    2.  **Tratamento de Nulos:** Preenchimento de 'Unknown' para desenvolvedores e nomes.
    3.  **Padronização de Datas:** Conversão para datetime (Default: `2000-01-01` para datas ausentes).
    4.  **Cálculo de Reviews:** Soma de reviews positivas e negativas.
    5.  **Sanitização:** Remoção de colunas duplicadas e garantia de tipos numéricos (preço, ccu, horas jogadas).
- **Saída:** `Silver/games_dataset.csv`.

### Camada Gold (Aggregated & Analytics)
...
1.  **Prepare o Ambiente:**
    Certifique-se de que sua estrutura de pastas local esteja assim (mapeada no `docker-compose.yaml`):
    ```text
    /seu-projeto
    ├── dags/
